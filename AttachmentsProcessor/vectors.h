...
struct lsclass vect_prototype2 : public DATA
{
	...
	template <typename T1, size_t S1, typename DATA1>
	vect_prototype2(const vect_prototype2<T1, S1, DATA>& other)
	{
		(*this)[i] = other[i];
	}
	...
	static vect_prototype2 fill(T v)
	{
		vect_prototype2 ret;
		for (size_t i = 0; i < csize; i++)
			ret[i] = v;
	...
	float rad(const vect_prototype2& to)
	{
		float s = 0;
		for (size_t i = 0; i < csize; i++)
			s += (float)( ((*this)[i] - to[i]) * ((*this)[i] - to[i]));
		s = pow(s, 1.0f / csize);
		return s;
	}

	double radd(const vect_prototype2& to, double p = 2.0)
	{
		double s = 0;
		for (size_t i = 0; i < csize; i++)
			s += (double)(  pow(abs((*this)[i] - to[i]), p) );
		s = pow(s, 1.0 / p);
		return s;
	}
